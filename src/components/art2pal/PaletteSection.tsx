...
export function PaletteSection({ palette, format, onFormatChange, onCopy }: PaletteSectionProps) {
  return (
    <section className="sp-panel sp-lift min-w-0 rounded-lg p-5">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        ...
        <p className="shrink-0 whitespace-nowrap text-sm font-semibold text-[rgb(33_44_51_/_0.56)]">{palette.colors.length} colors</p>
      </div>
      <div className="grid min-w-0 grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {palette.colors.map((color, index) => (
          <PaletteSwatch color={color} onCopy={onCopy} key={`${palette.id}-${index}-${color}`} />
        ))}
      </div>
      <ExportPanel colors={palette.colors} palette={palette} format={format} onFormatChange={onFormatChange} onCopy={onCopy} />
    </section>
  );
}
