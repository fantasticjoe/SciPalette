...
- **多维筛选**：支持按关键词、类别、图类型、颜色数量、自动计算的色盲友好性筛选；categorical 色板按可覆盖的最小类别数筛选，因此大色板可以用于较少类别的图。
...
构建后的站点会提供 `/palettes/catalog.json`，这是给外部脚本、Codex skill 和绘图工具使用的轻量目录。它只包含推荐所需的 `id`、名称、类别、HEX、用途、标签、色盲友好分数、背景和详情页 URL，不包含页面组件、源码文件或完整色盲模拟报告。

未来仓库即使增长到几百或几千个色板文件，外部推荐工具也不应该 clone 或扫描整个仓库；应优先读取线上 catalog，离线时使用随工具打包的 catalog snapshot。
